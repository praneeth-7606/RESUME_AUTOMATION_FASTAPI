...
# Check how to take the Templete of the resume 
...
output_dir = OUTPUT_DIR = os.path.join(BASE_PATH, "Output")
...
def LLM_CALL_1(resume_text: str) -> str:
    ...
    try:
        ...
        parsed_text = resume_text[:10000]  # Limit text length to avoid token limits
    ...
    Make sure the key values are same as in the text parsed. 
            """
...
def generate_resume_1(data_json):
    ...
    pdf.add_page()
    # pdf.logo_path = r"C:\Intern_Work\Resume_Automation\Resumate_Final\Resumate_Final\Resume_Automation\Backend\Logo.png"
    ...
    output_dir = r"C:\Intern_Work\Resume_Automation\Generated_Resumes"  # Ensure this directory exists
...
def generate_cover_letter_pdf(cover_letter_text: str, candidate_name: str) -> str:
    ...
    pdf.set_font("Arial", "B", 16)
    pdf.cell(0, 8, f"{candidate_name} ", ln=True, align="L")
...
@app.get("/")
def read_root():
    return {"message": "FastAPI is running!"}@app.get("/download/{filename}")
