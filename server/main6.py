...
import json
import os
# from resume_template2 import generate_resume_2  # Import the new template generator
...
class ResumeRequest(BaseModel):
    ...
    template_style: Optional[str] = "template1" 
    ...
    def projects_section(self, projects):
        if projects:
            ...
            for project in projects:
                self.set_font('Arial', 'B', 10)
                self.multi_cell(0, 6, self.clean_text(project['title']))
                self.set_font('Arial', '', 10)
                self.multi_cell(0, 6, self.clean_text(project['description']))
                self.ln(1)
...
def generate_resume_2(data):
    ...
    pdf.contact = " | ".join(filter(None, [data.get('phone', ''), data.get('email', ''), data.get('linkedin', '')]))
    pdf.add_page()
    ...
    if data.get('projects'):  # Matching key 'projects'
    ...
    # filename = f"{pdf.name.replace(' ', '_')}_Resume.pdf"
    # Update in generate_resume_2 function
    filename = os.path.join(OUTPUT_DIR, f"{pdf.name.replace(' ', '_')}_Resume.pdf")
    pdf.output(filename)
    return filename
...
# Add a route to serve the HTML pageal
...
#         # Step 3: Generate the resume PDF based on the selected template
#         try:
            
#             if request.template_style == "template2":
#                 if "project_details" in resume_data and isinstance(resume_data["project_details"], dict):

#         # Convert project_details dictionary to a list for template2
#                     projects = []
#                     for project_key, project_data in resume_data["project_details"].items():
#                         projects.append({
...
#             # Use your new template generator
#                 resume_path = generate_resume_2(json.dumps(resume_data))
#             else:
...
@app.post("/generate/resume", response_model=dict)
async def generate_resume(request: ResumeRequest, background_tasks: BackgroundTasks):
    """Generate a new resume based on template and input files using only template2"""
    # Validate that template exists
    if not os.path.exists(request.template_path):
        raise HTTPException(status_code=400, detail="Template file not found. Please upload a template first.")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                formatted_json_schema = {
                    "name": "",
                    "designation": "",
                    "summarized_objective": "",  # Changed from "objective" to match template2
                    "education": [],
                    "skills": [],
                    "projects": [],  # Changed from "project_details" to match template2
                    "technical_skills": {},  # Added for template2
                    "experience": [],  # Added for template2
                    "certifications": []  # Added for template2
                }
        except Exception as e:
            print(f"Error in LLM_CALL_1: {str(e)}")
            # Use default schema if LLM call fails - already formatted for template2
            formatted_json_schema = {
                "name": "",
                "designation": "",
                "summarized_objective": "",
                "education": [],
                "skills": [],
                "projects": [],
                "technical_skills": {},
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # If still failing, create minimal valid JSON formatted for template2
                    print("Error parsing cleaned JSON, using minimal data")
                    # Extract name from resume if available
                    name = "Candidate"
                    if old_resume_text:
                    ...
                    resume_data = {
                        "name": name,
                        "designation": "Professional",
                        "summarized_objective": "Experienced professional seeking new opportunities.",
                        "education": "Bachelor's degree in relevant field.",
                        "technical_skills": {
                            "languages": ["Skills not extracted"]
                        },
                        "projects": [
                            {
                                "title": "Project",
            ...
            print(f"Error in LLM call: {str(e)}")
            # Create minimal valid JSON formatted for template2 if LLM call fails
            name = "Candidate"
            if old_resume_text:
                for line in old_resume_text.split('\n'):
                    if line.strip() and len(line.strip()) < 50:
        ...
        # Transform data for template2 format
        # Convert standard resume format to template2 format
        transformed_data = resume_data.copy()
        
        # Handle objective/summarized_objective
        if "objective" in resume_data and not "summarized_objective" in resume_data:
            transformed_data["summarized_objective"] = resume_data["objective"]
        
        # Extract contact info from resume text if available
        if old_resume_text and not any(field in transformed_data for field in ["email", "phone", "linkedin"]):
            # Simple regex to find email
            email_match = re.search(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', old_resume_text)
            if email_match:
                transformed_data["email"] = email_match.group(0)
            
            # Simple regex to find phone number
            phone_match = re.search(r'\b\d{3}[-.]?\d{3}[-.]?\d{4}\b', old_resume_text)
            if phone_match:
                transformed_data["phone"] = phone_match.group(0)
        
        # Convert project_details dictionary to a list for template2
        if "project_details" in resume_data and isinstance(resume_data["project_details"], dict) and not "projects" in resume_data:
            projects = []
            for project_key, project_data in resume_data["project_details"].items():
                if isinstance(project_data, dict):
                    projects.append({
                        "title": project_data.get("name", ""),
                        "description": f"{project_data.get('description', '')} Technologies: {project_data.get('technology', '')}. Role: {project_data.get('role_played', '')}"
                    })
            transformed_data["projects"] = projects
        
        # Convert skills from list to categorized dict if needed
        if "skills" in resume_data and isinstance(resume_data["skills"], list) and not "technical_skills" in resume_data:
            transformed_data["technical_skills"] = {
                "technical": resume_data.get("skills", [])
            }
        
        # Step 3: Generate the resume PDF using template2
        try:
            resume_path = generate_resume_2(json.dumps(transformed_data))
        except Exception as e:
            print(f"Error generating PDF: {str(e)}")
            # Create a simple PDF with basic information
            from fpdf import FPDF
            ...
            pdf.add_page()
            pdf.set_font("Arial", "B", 16)
            pdf.cell(0, 10, transformed_data.get("name", "Candidate"), ln=True, align="L")
            pdf.set_font("Arial", "", 11)
            pdf.multi_cell(0, 7, "An error occurred while generating the complete resume.")
            pdf.ln(10)
            ...
            resume_path = os.path.join(OUTPUT_DIR, f"{transformed_data.get('name', 'Candidate').replace(' ', '_')}_Resume.pdf")
            pdf.output(resume_path)
        ...
        try:
            ...
            cover_letter_text = generate_cover_letter_from_resume(transformed_data)
            
            # Create a CoverLetterPDF class based on ResumePDF for consistent styling
            class CoverLetterPDF(ResumePDF):
                def header(self):
                    if self.first_page:
                        self.set_font('Courier', 'B', 25)
                        self.cell(0, 10, self.clean_text(self.name), align='C', ln=True)
            ...
            # Generate the cover letter using template2 styling
            pdf = CoverLetterPDF()
            pdf.name = transformed_data.get("name", "Candidate")
            pdf.contact = " | ".join(filter(None, [
                transformed_data.get('phone', ''),
                transformed_data.get('email', ''),
                transformed_data.get('linkedin', '')
            ]))
            pdf.add_page()
            ...
            # Save the cover letter
            cover_letter_path = os.path.join(OUTPUT_DIR, f"{pdf.name.replace(' ', '_')}_Cover_Letter.pdf")
