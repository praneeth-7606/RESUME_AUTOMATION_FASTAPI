...
def generate_resume_2(data):
    ...
    # Combine contact information if available
    contact_elements = []
    if data.get('phone'):
        contact_elements.append(data.get('phone'))
    if data.get('email'):
        contact_elements.append(data.get('email'))
    if data.get('linkedin'):
        contact_elements.append(data.get('linkedin'))
    pdf.contact = " | ".join(contact_elements) if contact_elements else ""
    ...
    if data.get('education'):
        ...
        # Handle education as either string or list
